...
export default class MiniGameModal extends Component {
    ...
    async getMinigamePage(id,game) {
        // const { user_energy, user_max_energy, user_id, cooldown_time } = this.state.user_data
        const { user_id } = this.state.user_data
        this.setState({ showModal: false }); 
        if(game==="game1"){
            console.log("play game1")
        }else if(game==="game2"){
            console.log("play game 2")
                window.location.replace("/MiniGame2?userId="+user_id);
        }
    ...
    render() {
        const { user_energy } = this.state.user_data
        return (
            <div>
